...
document.addEventListener('DOMContentLoaded', function() {
    @if(!empty($details['selected_dates']) && is_array($details['selected_dates']))
        const selectedDates = @json($details['selected_dates']);
        const datesVisual = document.getElementById('selectedDatesVisual');
        
        if (datesVisual && selectedDates.length > 0) {
            const dates = selectedDates.map(date => {
                const d = new Date(date);
                return d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
            }).join(', ');
            
            datesVisual.value = dates;
            datesVisual.classList.remove('d-none');
            datesVisual.classList.add('form-control', 'form-control-sm');
        }
    @endif
});
</script>
